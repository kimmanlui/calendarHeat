...
##
## This version is revised by Kimman Lui , 2018
##
...
calendarHeat <- function(dates, 
  ...
  #update NA values
  caldat$value[is.na(caldat$value)]=NAvalue; 
      ...
      if (panel.locs[row, column] > 0)
      {
        ...
        for (n in 1:51) {
          grid.lines(x = c(n + 1.5, n + 1.5), 
                     y = c(-0.5, 6.5), default.units = "native", gp=gpar(col = "black", lwd = 1))
        }
        x.start <- adj.start - 0.5
...
if (FALSE) 
{
  myDate=c('2018-02-02', '2017-02-02', '2007-02-02')
  myVal=c(10, 2, 20)
  calendarHeat(dates=myDate, values=myVal, varname="TEST")
